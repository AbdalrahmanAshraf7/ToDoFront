...
import useMissionDelete from '../../Hooks/MissionHooks/useMissionDelete.tsx'
import useToDo from '../../Hooks/MissionHooks/useToDoGet.tsx'
...
export default function TodoBody({searchValue}:any) {
  ...
  let {missions}:any = useToDo({missionName:searchValue.searchValue,date :searchValue.dropValue})
  let {fav,favLoading}:any = useMissionFav()
  let {deleteMission,deleteLodaing}= useMissionDelete()
...
{deleteLodaing&&itemIdDelete===M._id?<div  className='text-2xl text-white'>Lodaing...</div>:<i title='delete from missions' className="fa-solid fa-trash-can md:text-6xl text-4xl   text-red-800" onClick={(e)=>{e.stopPropagation() ; e.preventDefault() ;deleteMission(M._id) ; setItemIdDelete(M._id) }}></i>
}
      {favLoading&&itemId===M._id?<div className='text-2xl text-white'>Loading...</div>:
      <i title='add to favorit missions' className={`fa-solid fa-heart md:text-6xl text-5xl text-gray-300 relative ${M.fav ?"text-red-800" : null} `} onClick={(e)=>{e.stopPropagation() ; e.preventDefault() ; fav(M._id) ; setItemId(M._id)}}>
